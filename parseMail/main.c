...
int main(int argc, const char *argv[])
{
    ...
    FILE *inputtxt = fopen("/tmp/test.in", "r");
    ...
    snprintf(fnam, sizeof(fnam), "/tmp/testin.txt");
    fd_orig = fopen(fnam, "w");
    //snprintf(fnam, sizeof(fnam), "/tmp/o_unmime.%lx", (long)pid);
    snprintf(fnam, sizeof(fnam), "/tmp/testout.txt");
    ...
    *buf_p = '\0';
    ...
    do {
        ...
        *buf_p = '\0';
        ...
        DBG_FWRITE(buffer, (buf_p - buffer), 1, fd_orig);
        if (buf_p > buffer) {
